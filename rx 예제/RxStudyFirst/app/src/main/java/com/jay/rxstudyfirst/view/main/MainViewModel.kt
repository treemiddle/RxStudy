...
class MainViewModel(private val mainRepository: MainRepository) {
    private val sibal = MyApplication.getApplicationContext()
    ...
    private val ddd = PublishSubject.create<Unit>()
    ...
    private val ok = PublishSubject.create<Unit>()
    private val _check = MutableLiveData<Unit>()
    val check: LiveData<Unit> get() = _check

    init {
        Toast.makeText(sibal, "sibal", Toast.LENGTH_SHORT).show()
        rxBind()
    }
    ...
    private fun getMovie(query: String) {
        ...
        disposable = mainRepository.getMovies(query)
            ...
            .subscribe({ response ->
                ...
                _check.value = Unit
                Log.d("Main", "getMovie fail: ${t.message}")
    ...
    private fun rxBind() {
        ...
        ok.observeOn(AndroidSchedulers.mainThread())
            .subscribe { getMovie("man") }
            .let(compositeDisposable::add)
    }
